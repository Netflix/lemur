...
def test_publish(certificate, endpoint):
    ...
    message_ids = publish(
        topic_arn, data, "expiration", get_options(), region_name="us-east-1"
    )
    ...
    for certificate in data:
        ...
        assert actual_json["Message"] == format_message(
            certificate, "expiration", get_options()
        )
...
def test_send_expiration_notification_both_disabled():
    ...
    assert send_expiration_notifications([], ["aws-sns", "email-notification"]) == (
        0,
        0,
    )
