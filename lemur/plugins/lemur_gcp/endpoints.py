...
from google.cloud.compute_v1.services import ssl_policies, global_forwarding_rules, forwarding_rules, \
    target_https_proxies, target_ssl_proxies, region_target_https_proxies
from google.cloud.compute_v1 import TargetHttpsProxiesSetSslCertificatesRequest, \
    TargetSslProxiesSetSslCertificatesRequest, RegionTargetHttpsProxiesSetSslCertificatesRequest
...
def fetch_target_proxies(project_id, credentials, region):
    ...
    if region:
        ...
        http_proxies_client = region_target_https_proxies.RegionTargetHttpsProxiesClient(credentials=credentials)
...
def update_target_proxy_default_cert(project_id, credentials, endpoint, certificate, region):
    """
    ...
    if region and kind == "targethttpsproxy":
        current_app.logger.info(f"Rotating default cert for regional endpoint {endpoint.name} in {region}")
        client = region_target_https_proxies.RegionTargetHttpsProxiesClient(credentials=credentials)
        proxy = client.get(project=project_id, target_https_proxy=endpoint.name, region=region)
        set_region_target_https_proxy_certs(
            project_id, client, endpoint, [cert_self_link] + proxy.ssl_certificates[1:], proxy.ssl_certificates, region)
    elif not region and kind == "targethttpsproxy":
        current_app.logger.info(f"Rotating default cert for global endpoint {endpoint.name}")
...
def set_region_target_https_proxy_certs(
        ...
        old_self_links,
        region):
    current_app.logger.info(f"Setting certificates from {old_self_links} to {new_self_links} for "
...
def update_target_proxy_sni_certs(project_id, credentials, endpoint, old_cert, new_cert, region):
    kind = endpoint.type
    if kind not in ("targethttpsproxy", "targetsslproxy") or endpoint.registry_type != "gcp":
        raise NotImplementedError()
    ...
    if region and kind == "targethttpsproxy":
        current_app.logger.info(f"Rotating SNI cert for regional endpoint {endpoint.name} in {region}")
        client = region_target_https_proxies.RegionTargetHttpsProxiesClient(credentials=credentials)
        proxy = client.get(project=project_id, target_https_proxy=endpoint.name, region=region)
        certs = calculate_new_certs_for_sni_rotation(
            project_id, credentials, old_cert, proxy, new_cert_self_link, region)
        set_region_target_https_proxy_certs(project_id, client, endpoint, certs, proxy.ssl_certificates, region)
    elif not region and kind == "targethttpsproxy":
...
def calculate_new_certs_for_sni_rotation(project_id, credentials, old_cert, proxy, new_cert_self_link, region):
    cert_to_delete = certificates.find_cert(project_id, credentials, old_cert.body, proxy.ssl_certificates, region)
...
def fetch_forwarding_rules_map(project_id, credentials, region=None):
    ...
    if region:
        forwarding_rules_client = forwarding_rules.ForwardingRulesClient(credentials=credentials)
