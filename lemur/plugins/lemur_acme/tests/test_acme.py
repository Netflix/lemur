...
class TestAcme(unittest.TestCase):
    ...
    def test_get_dns_challenges(self, mock_len):
        ...
        result = yield self.acme._get_dns_challenges(host, mock_authz)
        self.assertEqual(result, mock_entry)

    def test_strip_wildcard(self):
        expected = ("example.com", False)
        result = self.acme._strip_wildcard("example.com")
    ...
    def test_authz_record(self):
        a = plugin.AuthorizationRecord("host", "authz", "challenges", "id")
        self.assertEqual(type(a), plugin.AuthorizationRecord)

    @patch("acme.client.Client")
    @patch("lemur.plugins.lemur_acme.plugin.current_app")
    @patch("lemur.plugins.lemur_acme.plugin.len", return_value=1)
    @patch("lemur.plugins.lemur_acme.plugin.AcmeHandler._get_dns_challenges")
    def test_start_dns_challenge(
        ...
        c = challenges.DNS01()
        mock_entry.chall = TestAcme.test_cleanup_dns_challenge_fail
        mock_authz.body.resolved_combinations.append(mock_entry)
        ...
        result = self.acme._start_dns_challenge(
    ...
    @patch("time.sleep")
    def test_cleanup_dns_challenge_success(
        ...
        self.acme._cleanup_dns_challenge(mock_acme, mock_authz)

    @patch("acme.client.Client")
    @patch("lemur.plugins.lemur_acme.plugin.current_app")
    @patch("lemur.plugins.lemur_acme.cloudflare.wait_for_dns_change")
    def test_cleanup_dns_challenge_fail(
        ...
        mock_authz.dns_challenge.append(dns_challenge)
        self.assertRaises(
            ValueError, self.acme._cleanup_dns_challenge(mock_acme, mock_authz)
        )
    ...
    @patch(
        "lemur.plugins.lemur_acme.plugin.AcmeHandler._start_dns_challenge",
        return_value="test",
    )
    def test_create_authorizations(self, mock_start_dns_challenge):
        mock_order = Mock()
        ...
        result = self.acme.create_authorizations(
            "acme_client", mock_order, mock_order_info
        )
        self.assertEqual(result, ["test"])

    @patch("lemur.plugins.lemur_acme.plugin.AcmeHandler._cleanup_dns_challenge")
    def test_verify_authorizations(self, mock_cleanup_dns_challenge):
        mock_authz = []
        mock_authz_record = MagicMock()
        mock_authz_record.authz = Mock()
        ...
        self.acme.verify_authorizations(mock_acme_client, mock_authz)
        self.assertEqual(mock_cleanup_dns_challenge.call_count, 1)

    @patch("lemur.plugins.lemur_acme.plugin.AcmeHandler._cleanup_dns_challenge")
    def test_remove_authorizations(self, mock_cleanup_dns_challenge):
        mock_authz = []
        mock_authz_record = MagicMock()
        mock_authz_record.authz = Mock()
        mock_authz_record.change_id = 1
        ...
        mock_authz.append(mock_authz_record)
        mock_acme_client = Mock()
        self.acme.remove_authorizations(mock_acme_client, mock_authz)
        self.assertEqual(mock_cleanup_dns_challenge.call_count, 1)

    # @patch("lemur.plugins.lemur_acme.plugin.AcmeHandler.mock_delete_txt_record")
    # @patch("lemur.plugins.lemur_acme.plugin.AcmeHandler.dns_provider_plugin.mock_delete_txt_record")
    """
    def test_cleanup_dns_challenge(self):
        mock_authz_record = MagicMock()
        mock_authz_record.authz = Mock()
        mock_authz_record.change_id = 1
        mock_authz_record.dns_challenge.validation_domain_name = Mock()
        ...
        mock_acme_client = Mock()
        self.acme.cleanup_dns_challenge(mock_acme_client, mock_authz_record)
        # self.assertEqual(mock_delete_txt_record.call_count, 1)
    """
    ...
    def test_get_dns_provider(
        ...
        route53 = provider._get_dns_provider("route53")
        assert route53
        cloudflare = provider._get_dns_provider("cloudflare")
        assert cloudflare
        dyn = provider._get_dns_provider("dyn")
        assert dyn
    ...
    @patch("lemur.plugins.lemur_acme.plugin.dns_provider_service")
    @patch("lemur.plugins.lemur_acme.plugin.AcmeHandler.create_authorizations")
    @patch("lemur.plugins.lemur_acme.plugin.AcmeHandler.verify_authorizations")
    ...
    def test_get_ordered_certificate(
        ...
        mock_verify_authorizations,
        mock_create_authorizations,
        mock_dns_provider_service,
        ...
        provider = plugin.ACMEIssuerPlugin()
        provider._get_dns_provider = Mock()
