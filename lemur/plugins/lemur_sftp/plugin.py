...
class SFTPDestinationPlugin(DestinationPlugin):
    ...
    options = [
        ...
        },
        {
            ...
            'type': 'str',
            'required': False,
            'helpMessage': 'The path to the RSA private key on the Lemur server. (not needed when the password is used)',
            'default': None
        },
        {
            'name': 'privateKeyPass',
    ...
    def upload(self, name, body, private_key, cert_chain, options, **kwargs):
        ...
        ssh_private_key = self.get_option('privateKeyPath', options)
        ssh_private_key_password = self.get_option('privateKeyPass', options)
        ...
        try:
            ...
            if password: 
                ...
                priv_key = paramiko.RSAKey.from_private_key_file(ssh_private_key, ssh_private_key_password)
                ssh.connect(host, username=user, port=port, pkey=priv_key)
            else:
