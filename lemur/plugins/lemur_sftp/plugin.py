...
class SFTPDestinationPlugin(DestinationPlugin):
    ...
    def upload_acme_token(self, token_path, token, options, **kwargs):
        ...
        current_app.logger.debug(
            "SFTP destination plugin is started for HTTP-01 challenge"
        )
            ...
            for filename, _ in files.items():
                ...
                try:
                    ...
                    if permerror.errno == 13:
                        current_app.logger.debug(
                            f"Deleting {filename} from {dst_path} returned Permission Denied Error, making file writable and retrying"
                        )
        ...
        try:
            ...
            for part in allparts:
                try:
                    ...
                    current_app.logger.debug(
                        f"{remote_path} doesn't exist, trying to create it"
                    )
                    try:
            ...
            for filename, data in files.items():
                ...
                try:
                    ...
                    if permerror.errno == 13:
                        current_app.logger.debug(
                            f"Uploading {filename} to {dst_path} returned Permission Denied Error, making file writable and retrying"
                        )
                        sftp.chmod(path.join(dst_path, filename), 0o600)
                        with sftp.open(path.join(dst_path, filename), "w") as f:
                            f.write(data)
