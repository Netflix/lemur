...
def create_user_roles(profile):
    ...
    # Automatically create roles from the user profile.
    # We do this to create an assign roles in Lemur dynamically and ad-hoc
    if current_app.config.get("IDP_GENERATE_ROLES_FROM_USER_GROUPS", False):
        idp_roles_description = current_app.config.get("IDP_ROLES_DESCRIPTION",
                                                       "Identity provider role (generated by Lemur)")
    ...
    if current_app.config.get("CREATE_PER_USER_ROLE", True):
