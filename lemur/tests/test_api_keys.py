...
def test_api_key_list_post_valid_self(client, user_id, token, status):
    assert client.post(api.url_for(ApiKeyList), data=json.dumps({'name': 'a test token', 'user_id': user_id, 'ttl': -1}), headers=token).status_code == status
...
def test_api_key_list_post_valid_no_permission(client, token, status):
    assert client.post(api.url_for(ApiKeyList), data=json.dumps({'name': 'a test token', 'user_id': 2, 'ttl': -1}), headers=token).status_code == status
